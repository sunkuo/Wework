...
export class EventsService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    const interval = this.configService.get<number>('EVENT_CLEANUP_INTERVAL', 60000);
  ...
  private sanitizeForLog(value: any, key = "", depth = 0): any {
    ...
    if (typeof value === "string") {
      ...
      if (key === "raw" || key === "payload") return this.truncateString(value, 1000);
      return this.truncateString(value, 240);
    }

    if (Array.isArray(value)) {
      const maxItems = 30;
      const out = value.slice(0, maxItems).map((item) => this.sanitizeForLog(item, "", depth + 1));
  ...
  async pushEvent(event: any) {
    ...
    try {
      ...
      if (err.code === 'P2003' && finalSessionUuid) {
        this.logger.warn(`[pushEvent] session ${finalSessionUuid} not found, saving as orphan event`);
        try {
          await this.prisma.event.create({
            data: {
              ...
              stage: `${stage} (orphan)`,
              event_type: eventType,
              payload: { ...sanitized, original_uuid: finalSessionUuid } as Prisma.InputJsonValue
            }
          });
        } catch (retryErr: any) {
          this.logger.error(`[pushEvent] orphan retry error: ${retryErr.message}`);
        }
      } else {
