...
export class WxWorkController {
  ...
  private buildInitPayload(body: any = {}) {
    ...
    return {
      ...
      proxySituation: Number.isFinite(proxySituationRaw)
        ? proxySituationRaw
        : 0,
  ...
  async getState(@Res() res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ ok: false, message: this.serializeError(error).message });
    }
  }
  ...
  async setCallback(@Body() body: any, @Res() res: Response) {
    try {
      ...
      if (!uuid) {
        return res
          .status(400)
          .json({ ok: false, message: '缺少 uuid，请先初始化' });
      }
      ...
      const callbackResp = await this.wxWorkService.setCallbackUrl(
        uuid,
        callbackUrl,
      );
      await this.eventsService.pushEvent({
  ...
  async checkCode(@Body() body: any, @Res() res: Response) {
    try {
      ...
      if (!uuid)
        return res
          .status(400)
          .json({ ok: false, message: '缺少 uuid，请先初始化登录' });
      if (!/^\d{6}$/.test(code)) {
        return res
          .status(400)
          .json({ ok: false, message: '验证码必须是6位数字' });
      }
      if (!qrcodeKey) {
        return res.status(400).json({
          ok: false,
          message: '缺少 qrcodeKey，请先获取二维码或手动输入',
        });
      }
  ...
  async refreshRunClient(@Body() body: any, @Res() res: Response) {
    try {
      ...
      const loginState = this.wxWorkService.extractLoginState(
        runByUuidResp.data,
      );
      if (loginState !== null) {
        await this.sessionService.updateActive({
  ...
  async getContacts(@Body() body: any, @Res() res: Response) {
    try {
      ...
      if (!uuid)
        return res
          .status(400)
          .json({ ok: false, message: '缺少 uuid，请先登录' });
      ...
      const externalLimit = normalizeLimit(
        externalReq.limit ?? body.externalLimit,
        100,
      );
      const innerStrSeq = String(innerReq.strSeq ?? body.innerStrSeq ?? '');
      const externalStrSeq = String(
        externalReq.strSeq ?? body.externalStrSeq ?? '',
      );
  ...
  async callback(@Body() body: any, @Res() res: Response) {
    try {
      ...
      const hasDisconnectSignal = (text: string) =>
        /手机端结束登录|其他设备登录|异常断开|断开|disconnect|offline/i.test(
          text,
        );
